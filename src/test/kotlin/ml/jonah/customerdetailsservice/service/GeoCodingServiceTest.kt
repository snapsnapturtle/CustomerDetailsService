...
internal class GeoCodingServiceTest {
    ...
    @Mock
    private lateinit var geoCodingServiceClient: GeoCodingServiceClient
    ...
    internal fun `should use feign client to resolve an address`() {
        ...
        val expectedGeoLocation = Coordinates(
            latitude = 20.0,
            longitude = 30.0
        )

        `when`(geoCodingServiceClient.getCoordinatesForAddress(AddressRequest(address))).thenReturn(geoCodingResponse)
